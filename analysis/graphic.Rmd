...
We divide the data into three section: all cancer site for overall cancer analysis among young adults, first class cancer site for specific cancer analysis. In this static graphs part, we will use the former two part to do the exploratory darta analysis.
...
g1 <- ggplot(cancer_year1,aes(x = year, y = count, group = 1)) + geom_line(col = "lightblue", size=1.5) + geom_point(size=2, shape=20, col = "red") + theme_solarized()
g1
```
...
g2 <- ggplot(cancer_age1, aes(x = age, y = count, fill = year)) + geom_bar(stat = "identity", position = "dodge") + scale_fill_brewer(palette = 4)+ theme(axis.text.x = element_text(size = 7, angle = 10)) + theme_solarized()
g2
```
...
g3 <- ggplot(cancer_sex1,aes(x = sex, y = count, fill = year)) + geom_bar(stat = "identity", position = "dodge") + scale_fill_brewer(palette = 4)+ theme(axis.text.x = element_text(size = 7, angle = 10)) + theme_solarized()
g3
```
...
g3 <- ggplot(cancer_race1,aes(x = race, y = rate, fill = year)) + geom_bar(stat = "identity", position = "dodge") + scale_fill_brewer(palette = 4)+ theme(axis.text.x = element_text(size = 7, angle = 10))+ theme_solarized()
g3
```
...
In the app, we show the results of cancer sites for different combination in age, sex and race. Users can choose these three variables according to their own interests through filter("Choose Conditions") in the left and then the app will show the most common cancer sites for that specific condition in a picture of body (generally 5 sites, if less than 5, then other sites' data for this is not recorded). At the mean time, below the picture, a barplot including all the recorded data is shown for more details. 
